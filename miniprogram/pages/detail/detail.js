...
Page({
      ...
      getPublish(e) {
            ...
            db.collection('publish').doc(e).get({
                  success: function(res) {
                    ...
                    that.getSeller(res.data._openid, res.data.bookinfo._id, sl);
                  }
            })
      },
  ...
  like() {
    ...
    if (likes){
      ...
      db.collection('likes').doc(that.data.publishinfo._id).remove({
        success: res => {
          wx.hideLoading();
          wx.showToast({
            title: '取消成功',
      ...
      creatOrder() {
            ...
            db.collection('order').add({
                  ...
                  success(e) {
                        that.history('购买', that.data.publishinfo.price, 2, e._id)
                  },
                  fail() {
                        wx.hideLoading();
                        wx.showToast({
